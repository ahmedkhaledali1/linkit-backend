...
exports.updateCustomOrder = catchAsync(async (req, res, next) => {
  ...
  if (req.body.companyInfo || req.body.orderDetails) {
    const updatedData = {
      companyInfo: { ...existingOrder.companyInfo, ...req.body.companyInfo },
      orderDetails: { ...existingOrder.orderDetails, ...req.body.orderDetails },
    };

    const validationError = validateCustomOrderFields(updatedData);
    if (validationError) {
      return next(new AppError(validationError, 400));
    }
  }
  ...
  const customOrder = await CustomOrder.findByIdAndUpdate(
    ...
    {
      new: true,
      runValidators: false, // We handle validation manually
    }
  );

  // Populate the updated order
  await customOrder.populate(getCustomOrderPopulationOptions());
